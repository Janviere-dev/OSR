...
export default function Auth() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'signin') {
      ...
      const { error } = await signUp(email, password, fullName, role, schoolData);
      if (error) {
        toast({ title: 'Error', description: error.message, variant: 'destructive' });
        setLoading(false);
      } else {
        toast({ title: 'Account created! Please check your email to verify your account.', description: 'After verifying, you can sign in.' });
        ...
        <div className="bg-card rounded-2xl shadow-card p-8 border border-border/50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <>
                ...
                <div>
                  ...
                  >
                    <div className="flex items-center space-x-2 p-2 rounded-lg border border-border hover:bg-muted/50 transition-colors cursor-pointer">
                      <RadioGroupItem value="parent" id="parent" />
                      <Label htmlFor="parent" className="cursor-pointer font-medium">{t('auth.role.parent')}</Label>
                    </div>
                    <div className="flex items-center space-x-2 p-2 rounded-lg border border-border hover:bg-muted/50 transition-colors cursor-pointer">
                      <RadioGroupItem value="school_admin" id="school_admin" />
                      <Label htmlFor="school_admin" className="cursor-pointer font-medium">{t('auth.role.school')}</Label>
                    </div>
                  </RadioGroup>
                </div>
                ...
                {role === 'school_admin' && (
                  <div className="space-y-4 p-4 bg-secondary/50 rounded-lg border border-border">
                    ...
                    <div>
                      ...
                      <Input id="schoolName" value={schoolName} onChange={(e) => setSchoolName(e.target.value)} required className="mt-1 input-focus" placeholder={t('auth.schoolNamePlaceholder')} />
                    </div>

                    <div>
                      <Label htmlFor="staffRole">{t('auth.school.staffRole') || 'Your Role in School'}</Label>
                      <Input id="staffRole" value={staffRole} onChange={(e) => setStaffRole(e.target.value)} className="mt-1 input-focus" placeholder={t('auth.staffRolePlaceholder')} />
                    </div>

                    <div>
                      <Label htmlFor="qualifications">{t('auth.school.qualifications') || 'Qualifications'}</Label>
                      <Input id="qualifications" value={qualifications} onChange={(e) => setQualifications(e.target.value)} className="mt-1 input-focus" placeholder={t('auth.qualificationsPlaceholder')} />
                    </div>

                    <div>
                      <Label htmlFor="schoolDesc">School Description</Label>
                      <textarea id="schoolDesc" className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm min-h-[80px] input-focus" placeholder={t('auth.schoolDescPlaceholder')} />
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div>
                        ...
                        <Select value={province} onValueChange={setProvince}>
                          <SelectTrigger className="mt-1"><SelectValue placeholder={t('auth.selectProvince')} /></SelectTrigger>
                      ...
                      <div>
                        ...
                        <Select value={district} onValueChange={setDistrict} disabled={!province}>
                          <SelectTrigger className="mt-1"><SelectValue placeholder={t('auth.selectDistrict')} /></SelectTrigger>
                      ...
                      <div>
                        ...
                        <Select value={sector} onValueChange={setSector} disabled={!district}>
                          <SelectTrigger className="mt-1"><SelectValue placeholder={t('auth.selectSector')} /></SelectTrigger>
                    ...
                    <div>
                      ...
                      {logoPreview ? (
                        <div className="mt-2 relative inline-block">
                          <img src={logoPreview} alt="School logo preview" className="w-24 h-24 object-contain rounded-lg border border-border" />
            ...
            <div>
              ...
              <Input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 input-focus" />
            </div>

            <div>
              <Label htmlFor="password">{t('auth.password')}</Label>
              <Input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} className="mt-1 input-focus" />
            </div>

            {mode === 'signup' && (
              <div>
                <Label htmlFor="confirmPassword">{t('auth.confirmPassword') || 'Confirm Password'}</Label>
                <Input id="confirmPassword" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required minLength={6} className="mt-1 input-focus" />
              </div>
            )}
